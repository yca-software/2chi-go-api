...
`)
	...
	t.Setenv(EnvPrefix+"AUTH__ACCESS_TOKEN_SECRET", "access-secret")
	t.Setenv(EnvPrefix+"AUTH__TOKEN_HASH_PEPPER", "pepper")
	...
	if got, want := cfg.Auth.AccessTokenSecret, "access-secret"; got != want {
		t.Fatalf("cfg.Auth.AccessTokenSecret = %q, want %q", got, want)
	}
	if got, want := cfg.Auth.TokenHashPepper, "pepper"; got != want {
